...
Одно отличие от этого документа: `is_correct` в списке вопросов не отдаётся —
иначе правильные ответы видно во вкладке «Сеть» браузера. Правильность
узнаётся запросом проверки.
...
| `/api/v1/modules/<id>/test/` | GET | активный тест блока: `id`, `title`, `question_count`, `duration_minutes` |
...
- Тест без вопросов на сайт не пускают. В блоке показывается ровно один тест:
  это держит частичный уникальный индекс в базе, а формы и кнопка «показать»
  объясняют, какой именно тест занимает место.
